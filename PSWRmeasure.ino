...
void adc_poll_and_feed_circular(void)
{
  ...
  uint8_t           i=0;
  #endif
  //ADC::Sync_result  result;                   // A variable containing the result of synchronous
...
void calc_SWR_and_power(void)
{
  ...
  if ((power_mw > MIN_PWR_FOR_SWR_CALC) || (power_mw < -MIN_PWR_FOR_SWR_CALC))
  {
    ...
    if (swr >= 10 ){swr = 10.01;}  //rounding to 10.0
    ...
    {
      //if (flag.swr_alarm && flag.config_mode == false)                        // This is the second time in a row, raise the SWR ALARM
      
              
        //if (flag_swr_bg == false){   tft.fillRect(300,0,162,80, RED);flag_swr_bg =true;
          //VirtLCDAlarm.clear();
...
void determine_power_pep_pk(void)
{
  ...
  static uint32_t a;                            // Pk:  100ms ring buffer counter
  static uint32_t b;                            // PEP: ring buffer counter
  static uint32_t c;                            // Long: ring buffer counter
  static uint32_t d;                            // avg: short ring buffer counter
  static uint32_t e;                            // avg: 1s ring buffer counter
  ...
  if (fwd_power_mw > 9000000) fwd_power_mw = 9000000;
  if (ref_power_mw > 9000000) ref_power_mw = 9000000;
  ...
  f_inst = (double) fwd * adc_ref/4096;
  ...
  //
  // Establish actual measured voltage at diode
  r_inst = (double) rev * adc_ref/4096;
  // Convert to VRMS in Bridge
  ...
  pk =-0x7fffffff;
  for (uint16_t x = 0; x < BUF_SHORT; x++)
  {
