...
const char *display_bat_menu_items[] =
          {  "Display Rotate",
             ...
             "Display Dim Toggle",
             #else
             "Backlight Adjust", // Backlight Adjust
             #endif
             "Screensaver Threshold",
             "Low Power Floor",
             "Go Back",
...
const uint8_t modscope_menu_size = 12;
    ...
    if (lines == 4)
    {
      ...
      // 5th item
       if (current_choice == menu_size-1) 
        ...
        x = 1;
      else if (current_choice == menu_size - 3 ) 
        x = 0;
      else 
        x = current_choice + 3;
        
      VirtLCDw.setCursor(begin_col, begin_row+4);
      for (a = begin_col; a < 20; a++)
       VirtLCDw.print(" ");
       VirtLCDy.print(" ");
      
      VirtLCDw.setCursor(begin_col + 2, begin_row + 4);
      sprintf(lcd_buf,"%s", *(menu + x));
      VirtLCDw.print(lcd_buf);
//---------------------------------------------------------------------------
      ...
      // 6th item
       if (current_choice == menu_size-1) 
        ...
        x = 1;
      else if (current_choice == menu_size - 4 ) 
        x = 0;
      else 
        x = current_choice + 4;
 
      VirtLCDw.setCursor(begin_col, begin_row+5);
      for (a = begin_col; a < 20; a++)
       VirtLCDw.print(" ");
       VirtLCDy.print(" ");
      
      VirtLCDw.setCursor(begin_col + 2, begin_row + 5);
      sprintf(lcd_buf,"%s", *(menu + x));
      VirtLCDw.print(lcd_buf);
     if (flag_display_functions == 0){
  //------------------------------------------------------------------------------           
      // 7th item
       if (current_choice == menu_size-1) 
        x = 4;
        ...
        x = 1;
     else if (current_choice == menu_size - 5 ) 
        x = 0;
      else 
        x = current_choice + 5;
         
      VirtLCDw.setCursor(begin_col, begin_row + 6);
      for (a = begin_col; a < 20; a++)
        VirtLCDw.print(" ");
        VirtLCDy.print(" ");
      
      VirtLCDw.setCursor(begin_col + 2, begin_row + 6);
      sprintf(lcd_buf,"%s", *(menu + x));
      VirtLCDw.print(lcd_buf);
 //------------------------------------------------------------------------------      
     // 8th item
     if (current_choice == menu_size-1) 
        ...
        x = 1;
     else if (current_choice == menu_size - 6 ) 
        x = 0;
      else 
        x = current_choice + 6;
  
      VirtLCDw.setCursor(begin_col, begin_row + 7);
      for (a = begin_col; a < 20; a++)
        VirtLCDw.print(" ");
        VirtLCDy.print(" ");
      
      VirtLCDw.setCursor(begin_col + 2, begin_row + 7);
      sprintf(lcd_buf,"%s", *(menu + x));
      VirtLCDw.print(lcd_buf);
 //--------------------------------------------------------------------------------     
     ...
     if (current_choice      == menu_size  -1) 
        ...
        x = 2;
     else if (current_choice == menu_size - 7 ) 
        x = 1; 
     else if (current_choice == menu_size - 8 ) 
        x = 0;  
      else 
        x = current_choice + 7;
      ...
      VirtLCDw.setCursor(begin_col, begin_row + 8);
      for (a = begin_col; a < 20; a++)
        VirtLCDw.print(" ");
        VirtLCDy.print(" ");
      
      VirtLCDw.setCursor(begin_col + 2, begin_row + 8);
      sprintf(lcd_buf,"%s", *(menu + x));
      VirtLCDw.print(lcd_buf);
     ...
     if (current_choice      == menu_size  -1) 
        ...
        x = 1;  
     else if (current_choice == menu_size - 9 ) 
        x = 0;  
      else 
        x = current_choice + 8;
      ...
      VirtLCDw.setCursor(begin_col, begin_row + 9);
      for (a = begin_col; a < 20; a++)
        VirtLCDw.print(" ");
        VirtLCDy.print(" ");
      
      VirtLCDw.setCursor(begin_col + 2, begin_row + 9);
      sprintf(lcd_buf,"%s", *(menu + x));
      VirtLCDw.print(lcd_buf);
...
void debug_menu(void)
{
  ...
  if (flag.short_push)
  {
    ...
    Menu_exit_timer = 1;                // Timer to Exit and clear screen
...
void swr_alarm_threshold_menu(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    VirtLCDy.setCursor(5,0);
    VirtLCDy.print("SWR Alarm Threshold");
    VirtLCDy.setCursor(3,4);
    VirtLCDy.print("Adjust >>  ");
    VirtLCDw.setCursor(11,4);
    ...
    VirtLCDw.setCursor(0,7);
    VirtLCDw.print("Range is 1.5 to 3.9");
    VirtLCDw.setCursor(0,8);
...
void swr_alarm_power_threshold_menu(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    VirtLCDw.clear();
    VirtLCDy.clear();
    VirtLCDy.setCursor(6,0);
    VirtLCDy.print("SWR Alarm Power:");
//----------------------------------------------------------------------------------------------------------------------------------
    // Print the Rotary Encoder scroll Menu
    lcd_scroll_Menu((char**)swr_alarm_pwr_thresh_menu_items, menu_size, current_selection, 3, 7,3);
...
void pep_menu(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    // Print the Rotary Encoder scroll Menu
    lcd_scroll_Menu((char**)pep_menu_items, menu_size, current_selection, 3, 7,3);
    VirtLCDy.setCursor(0,4);
    VirtLCDy.print("Select");
...
//--------------------------------------------------------------------
void displaytft_menu()  // displayrotate_menu()
{
  R.disp.tft_on = display_attached;                  // XOR
  EEPROM_writeAnything(1,R);              // Write default settings into EEPROM
  
  VirtLCDw.clear();
  VirtLCDy.clear();
  VirtLCDy.setCursor(4,1);        
  VirtLCDy.print("   Not implemented!");
  VirtLCDy.transfer();  
  
  Menu_exit_timer = 100;                   // Show on LCD for 2 seconds
...
void displaydim_menu(void)
{
  ...
  if (Enc.read()/ENC_RESDIVIDE != 0)
  {
    if ((Enc.read()/ENC_RESDIVIDE > 0) && (R.disp.tft_backlight > 0) )  
    {
      R.disp.tft_backlight--;
    }
    if ((Enc.read()/ENC_RESDIVIDE < 0) && (R.disp.tft_backlight < 10) ) 
    {
      R.disp.tft_backlight++;
    }
    // Reset data from Encoder
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    VirtLCDw.setCursor(12,4);
    VirtLCDw.print(lcd_buf);
        
    VirtLCDw.setCursor(4,8);
    VirtLCDw.print("0 min, 10 max");
    tft_brightness=(R.disp.tft_backlight *15 +100); // minimal 100 and maximal 250
  } 
  // Enact selection by saving in EEPROM
  if (flag.short_push)
  {
...
void screensaverthresh_menu(void)
{
  ...
  if (R.idle_disp_thresh == 0) current_selection = 0;
   else if (R.idle_disp_thresh * 1000 == 1) current_selection = 1;
   else if (R.idle_disp_thresh * 1000 == 10) current_selection = 2;
   else if (R.idle_disp_thresh * 1000 == 100) current_selection = 3;
   else if (R.idle_disp_thresh * 1000 == 1000) current_selection = 4;
   else if (R.idle_disp_thresh * 1000 == 10000) current_selection = 5;
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    VirtLCDy.clear();
    VirtLCDy.setCursor(4,0);
    VirtLCDy.print("Screensaver Threshold");
    //VirtLCDy.setCursor(5,1);
    //VirtLCDy.print("Threshold:");

    // Print the Rotary Encoder scroll Menu
    lcd_scroll_Menu((char**)screenthresh_menu_items, menu_size, current_selection, 3, 7,3);
    VirtLCDy.setCursor(0,4);
    VirtLCDy.print("Select");
...
void lowpowerfloor_menu(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    // Print the Rotary Encoder scroll Menu
    lcd_scroll_Menu((char**)lowpowerfloor_menu_items, menu_size, current_selection, 3, 7,3);
    VirtLCDy.setCursor(0,4);
    VirtLCDy.print("Select");
    ...
    VirtLCDw.setCursor(0,8);
    VirtLCDw.print("indication on display");
  }
...
void display_batt_menu(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    VirtLCDy.setCursor(5,0);
    VirtLCDy.print("Display functions");
    flag_display_functions = 1;
          
    
    lcd_scroll_Menu((char**)display_bat_menu_items, menu_size, current_selection,2, 0,4);
  ...
  if (flag.short_push)
  {
    ...
    switch (current_selection)
    {
      ...
      case 4:
        ...
        #else
        VirtLCDy.setCursor(5,1);
        VirtLCDy.print("Done with Display");
...
void scalerange_menu_level2(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    VirtLCDw.setCursor(5,4);
    sprintf(lcd_buf,"1st Range = %2u",R.ScaleRange[0]);
    VirtLCDw.print(lcd_buf);
    VirtLCDw.setCursor(5,5);
    ...
    // Place "===>" in front of the "ScaleRange" currently being adjusted
    VirtLCDy.setCursor(1,scale_set + 4);
    VirtLCDy.print("===>");
  }
  #endif
...
void scalerange_menu(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    {
       VirtLCDy.setCursor(0,0);
      VirtLCDy.print("Up / Down or Select to exit ");
    ...
    if (current_selection<3)
    {
      VirtLCDy.setCursor(2,current_selection+4);
      VirtLCDy.print(">>");
    }
  }
  ...
  if (flag.short_push)
  {
    ...
    switch (current_selection)
    {
      ...
      default:
        ...
        VirtLCDy.clear();
        VirtLCDy.setCursor(4,1);
        VirtLCDy.print("Done with Scale Ranges");
        Menu_exit_timer = 100;                           // Show on LCD for 1 second
        flag.idle_refresh = true;                        // Force screensaver reprint upon exit
        flag.config_mode = true;                         // Not done yet, bac to Config Menu
        //flag.mode_change = true;                         // Signal for display cleanup
...
void modscope_menu(void)
{
  ...
  // Get Current value
  if (R.modscopeDivisor > 10) current_selection = 11;  /// Valid are 1 - 10 and 20
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    if      (current_selection == 11) R.modscopeDivisor = 20;
    else R.modscopeDivisor = current_selection;

    VirtLCDw.clear();
    VirtLCDy.clear();
    VirtLCDy.setCursor(0,0);
    VirtLCDy.print("Modulation Scope scan Period:");
   ...
   // VirtLCDy.print("Scan Period:");
    ...
    // Print the Rotary Encoder scroll Menu
    lcd_scroll_Menu((char**)modscope_menu_items, menu_size, current_selection, 3, 7,3);

    VirtLCDw.setCursor(0,7);
    VirtLCDw.print("Available Scan Rates");
    VirtLCDw.setCursor(0,8);
    VirtLCDw.print("0.04 to 0.8 seconds");
...
void calibrate_menu_level2(void)
{
 ...
 if (Enc.read()/ENC_RESDIVIDE != 0)
  ...
  if(!flag.menu_lcd_upd)  
  {
    ...
    int16_t min_value = -200;                            // Lowest permissible Calibration value in dBm * 10
    ...
    VirtLCDw.clear();
    VirtLCDy.clear();
    VirtLCDy.setCursor(2,0);  

    if (menu_level == CAL_SET0_MENU)                     // equals cal_set == 0
    {
      VirtLCDy.print("Single Level Cal:"); status_ind_cal =4;
    }
    else if (menu_level == CAL_SET1_MENU)                // equals cal_set == 1
    {
      VirtLCDy.print("First Cal SetPoint:"); status_ind_cal = 5; 
    }
    else if (menu_level == CAL_SET2_MENU)
    {
      VirtLCDy.print("Second Cal SetPoint:");status_ind_cal = 6; 
    }
    ...
    if (current_selection < 0)
    {
      ...
      sprintf(lcd_buf_cal, " -%1u.%01u", val, val_sub); // 
    }
    else
    {
      sprintf(lcd_buf," %2u.%01u",val, val_sub);
      sprintf(lcd_buf_cal, " %2u.%01u", val, val_sub);  
    }
    VirtLCDw.print(lcd_buf);

    if (cal_sig_direction_quality == CAL_FWD)
    {
      VirtLCDw.setCursor(3,9);
...
void calibrate_menu(void)
{
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    VirtLCDcal.clear();
    // Print the Rotary Encoder scroll Menu
    lcd_scroll_Menu((char**)calibrate_menu_items, menu_size, current_selection,3, 0,3);
    ...
    if (current_selection <= 2)
    {
      ...
      VirtLCDcal.setCursor(0,0);
      if (value < 0)
      {
        val*=-1;
        val_sub*=-1;
        sprintf(lcd_buf,"-%1u.%01u",val, val_sub);
        sprintf(lcd_buf_actual, "-%1u.%01u", val, val_sub);
      }
      else
      {
        sprintf(lcd_buf,"%2u.%01u",val, val_sub);
  ...
  if (flag.short_push)
  {
    ...
    switch (current_selection)
    {
      ...
      default:
        ...
        VirtLCDy.clear();
        VirtLCDy.setCursor(5,1);        
        VirtLCDy.print("Done with Calibration");
        Menu_exit_timer = 100;                           // Show on LCD for 1 second
        flag.idle_refresh = true;                        // Force screensaver reprint upon exit
        flag.config_mode = true;                         // We're NOT done, just backing off
        //flag.mode_change = true;                         // Signal for display cleanup
...
void factory_menu(void)
{
  ...
  if (flag_reset == 1){ Enc.write(-4);flag_reset =0;} // default No - Go Back in Factory Reset Menu (avoid mistakes)
  ...
  if (!flag.menu_lcd_upd)
  {
    ...
    // Print the Rotary Encoder scroll Menu
    lcd_scroll_Menu((char**)factory_menu_items, menu_size, current_selection, 4, 0,2);
    
    VirtLCDw.setCursor(0,8);
    VirtLCDw.print(" Factory Reset Menu");
    VirtLCDw.setCursor(0,9);
    VirtLCDw.print(" Calibration deleted");
    VirtLCDy.setCursor(0,  4); // remove old cursor
    VirtLCDy.print("  ");  
  ...
  if (flag.short_push)
  {
    ...
    switch (current_selection)
    {
      case 0:                                            // Factory Reset
        ...
        VirtLCDw.clear();
        VirtLCDy.clear();
        VirtLCDy.setCursor(3,2);        
        VirtLCDy.print("Factory Reset");
        VirtLCDy.setCursor(4,5);
        VirtLCDy.print("All default");
        
        Menu_exit_timer = 300;                           // Show on LCD for 2 seconds
        flag.idle_refresh = true;                        // Force screensaver reprint upon exit
        VirtLCDw.transfer();
        VirtLCDy.transfer();
        SOFT_RESET();
...
void ConfigMenu(void)
{
  ...
  else if (menu_level == DISPLAYTFT)     displaytft_menu();
