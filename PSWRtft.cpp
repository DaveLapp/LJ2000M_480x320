...
{  
  ...
  tft.setCursor( x+len - 4*strlen(range), y+height+15);
  tft.print(range);                  
}
...
{  
  ...
  tft.setFont(DroidSansMono_12);  //16
  tft.setTextColor(colour);
  tft.drawFastVLine( x+1, y+height+2, 5, colour);   // Draw the Scale
  tft.setCursor( x-1, y+height +15);                  // 1st value
  tft.print("0");     
  ...
  for (i=len/divisor, j=scale/divisor; i<=len; i+=len/divisor, j+=scale/divisor)
  {
    ...
    else
    {
      ...
      tft.setCursor( x+i - offs, y+height+15);
...
{  
  ...
  //ztft.drawRect(x, y,len+2, +2, colour); // weird?????
   tft.drawRect(x, y,len+2, height+2, colour); // good
  ...
  { 
    ...
    //Serial.println(offs);
    tft.setCursor(offs -5, y+height+15);                
    if (scalemark[i]<10) tft.print(scalemark[i], subdecimal);
    else tft.print("SWR");                  // Print indicator at highest mark      
  }
...
void TextBox::init(int16_t _col, int16_t _row, int16_t _xoffs, int16_t _yoffs, 
                   const ILI9488_t3_font_t &f, int16_t _fontColour, int16_t _blnkColour)
{
  xoffs =      _xoffs; 
  yoffs =      _yoffs;
  Col   =      _col;
...
void TextBox::clear(void)
{
  ...
  for (uint16_t i=0; i<lcdsize; i++) virt_lcd[i]=' ';  // Print a lot of spaces to virt LCD
  virt_x = 0;
  virt_y = 0;
}
