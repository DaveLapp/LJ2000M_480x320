...
#define                VERSION "2.06a"
#define                DATE    "09-01-22"
//**
//*********************************************************************************
#define _PSWR_T_H_
#define _PSWR_T_H_
...
//-----------------------------------------------------------------------------
// Enable Touch Screen 
#define TOUCHSCREEN_ENABLED      1 // 1 to enable, else 0.
//-----------------------------------------------------------------------------
// FT5206 Capacitive Touch Controller or a Resistive XPT2046 Touch Controller
#define FT5206_INSTALLED          0 // 1 For Cap Touch Controller, else 0
//-----------------------------------------------------------------------------
...
#define Wire_ENABLED              1 // 1 to enable, 0 to disable

//-----------------------------------------------------------------------------
// Defaults for measurement rate, PEP, PK and AVG buffer sizes
                                // (Overclocking can be enabled by editing board.txt in Teensyduino)
#define SAMPLE_TIMER              50  // Interrupt timer, in microseconds //667
#define BUF_SHORT               1000  // Buffer size for 100ms Peak 150
#define PEP_BUFFER               500  // PEP Buffer size in multiples of BUF_SHORT, can hold up to 5 second PEP
#define LONG_BUFFER               30  // LONG Buffer size in multiples of shortest PEP period (1s) - typically 30 seconds
#define AVG_BUF1S              10000  // AVG Buffer sample size (1000 is 1 second, if SAMPLE_TIMER = 1000)
#define AVG_BUFSHORT            1000  // Short (100ms) AVG Buffer size
#define AVG_BUFSWR                25  // Very short buffer for SWR bargraph smoothing.  Time is POLL_TIMER*AVG_BUFSWR
#define MODSCOPE_DIVISOR           1  // Modultion Scope scan rate divisor //1
 
//-----------------------------------------------------------------------------
// Timer for Power/SWR calculations and LCD prints, milliseconds
#define POLL_TIMER               10 // Meter updates will be at half this rate //10
...
//
#define LOGAMP1_SLOPE          25 // mV/dB.  Used with Single Level Calibration //23.5
#define LOGAMP2_SLOPE          25 // mV/dB.  Used with Single Level Calibration
...
#define TWENTYTOONE               1 // 1 to select the values for 20 to 1 coupler, else 0
//
...
#define BRIDGE_COUPLING       24.0  // Bridge Coupling
...
#if AD8307_INSTALLED
#define STARTUPDISPLAY1         "2 x AD8307 Based"
#else
...
#define SLEEPMSG   "NO SIGNAL" // Shown when nothing else to display on LCD
...
#define MODE_INTRO_TIME         50 // 100ths of a second (200 equals 2s)

//-----------------------------------------------------------------------------
// LED Blink
//
// None, or only one of the three below should be selected
#define INTR_LOOP_THRU_LED       1  // Blink the debug led on Microprocessor while sampling the AD within the interrupt function *OR*
...
const int R_Led               =  22;  // SWR Alarm
const int X_Led               =  23;  // Debug
...
typedef struct {
          int16_t  fwd[10256];                  // Circular buffer of Forward measurement values
          int16_t  rev[12056];                  // Circular buffer of Reverse measurement values
...
typedef struct  {
          unsigned tft_on                      ;   // Upside Down or Not
...
extern ILI9488_t3 tft;                        // To keep the .cpp stuff happy
